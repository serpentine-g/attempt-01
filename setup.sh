...
if [ "$BASIC" = true ] ; then
    ...
    # Install image libraries before pillow-simd for proper codec support
    sudo apt install -y libjpeg-dev libwebp-dev libwebp7
    # Uninstall any existing pillow versions to avoid conflicts
    pip uninstall -y pillow pillow-simd 2>/dev/null || true
    # Install pillow-simd with WebP support
...
if [ "$OVOXEL" = true ] ; then
    # Initialize Eigen submodule if not already done
    if [ ! -f "o-voxel/third_party/eigen/CMakeLists.txt" ]; then
        echo "[O-VOXEL] Initializing Eigen submodule..."
